...
class Network():
    ...
    def inNet(self, value):
        if self._netIDByNode.has_key(value):
            return True
        else:
            return False
    ...
    def getNetID(self, node):
        if self._netIDByNode.has_key(node):
...
class Seg:
    ...
    def __repr__(self):
        ...
        return "Seg(%(id)r, %(node1)r, %(node2)r, %(weight)r)" %vars()
    ...
    def __str__(self):
        ...
        node2 = self.getNode2()
        weight = self.getWeight()
        return "(%(id)s, %(node1)s, %(node2)s, %(weight)s)" %vars()
